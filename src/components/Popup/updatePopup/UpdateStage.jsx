...
import AddForm from "layouts/page/stage/AddForm";
...
export default function UpdateStage() {
  ...
  useEffect(() => {
    const callAPI = async () => {
      await dispatch(getStagebyId(id, token));
    };
    callAPI();
  }, [dispatch, id]);

  const stagedetail = useSelector((state) => {
    return state.stageByid;
  });
  ...
  const handleSubmitForm = async () => {
    ...
    try {
      const res = await API("PUT", URL_API + `/api/v1/stages/${id}`, data, token);
      CustomizedToast({
        message: `Cập nhật giai đoạn thành công ${title}`,
        type: "SUCCESS",
      });
