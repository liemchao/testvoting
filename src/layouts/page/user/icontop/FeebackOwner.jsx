...
import { getFeeback } from "context/redux/action/action";
...
export default function FeedbackOwer() {
  ...
  React.useEffect(() => {
    const callAPI = async () => {
      await dispatch(getFeedBackCampaign(id, token));
    };
    callAPI();
  }, [dispatch, token]);

  const feedback = useSelector((state) => {
    return state.feedbackcampaign;
  });
